...
#                    return await self.bot.connection.inform(data, destinations=self.bot.ipc.sources)
    ...
    async def get_tracking_channel(self):
        ...
        try:
            results = await self.bot.db.fetch("SELECT channel_id FROM tracking_channels")
    ...
    @username.command(name = "channel", aliases = ["set"], brief = "Set the channel where username changes will be sent.", example = ",channel #username-changes")
    ...
    async def channel(self, ctx, channel: discord.TextChannel):
        ...
        try:
            ...
            if not channel.permissions_for(ctx.guild.me).send_messages:
                return await ctx.fail(f"I don't have permission to send messages in {channel.mention}")
            ...
            await ctx.success(f"Username change notifications will now be sent to {channel.mention}.")
            self.logger.info(f"Tracking channel set to {channel.id} for guild {ctx.guild.id}")
            
        except Exception as e:
            self.logger.error(f"Failed to set tracking channel for guild {ctx.guild.id}: {e}")
            await ctx.fail(f"An error occurred while setting the tracking channel: {e}")

    @username.command(name = "unset", aliases = ["remove", "reset"], brief = "Remove the channel where username changes are sent.", example = ",unset")
    @commands.has_permissions(manage_channels=True)
    async def unset(self, ctx):
        ...
        try:
            ...
            self.logger.error(f"Failed to unset tracking channel for guild {ctx.guild.id}: {e}")
