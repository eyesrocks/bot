    ...
    def __init__(self, bot):
        ...
        async def get_caption(
            ...
            def do_caption(para, image, message):
                ...
                fnt = ImageFont.truetype("quote/Arial.ttf", 32)
                current_h, pad = 120 - (int(len(para) / 2) * 32) + len(para) * 5, 0

                for line in para:
                    bbox = draw.textbbox((0, 0), line, font=fnt)
                    w3, h3 = bbox[2] - bbox[0], bbox[3] - bbox[1]
                    draw.text(((680 - w3) / 2, current_h), line, font=fnt, fill="#FFF")
                    current_h += h3 + pad

                dr = ImageDraw.Draw(sa)
                font = ImageFont.truetype("quote/Arial.ttf", 20)
                author_text = f"- {str(message.author)}"
                bbox = dr.textbbox((0, 0), author_text, font=font)
                authorw = bbox[2] - bbox[0]
                dr.text((680 - authorw - 20, current_h + 10), author_text, font=font, fill="#FFF")
