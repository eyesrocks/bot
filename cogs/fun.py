...
class Fun(commands.Cog):
    ...
    async def vape(self, ctx):
        ...
        await self.bot.db.execute("""
            INSERT INTO vape_hits (user_id, guild_id, hits, has_vape)
            VALUES ($1, $2, 1, TRUE)
        ...
        await self.bot.db.execute("""
            UPDATE vape_hits
            SET guild_hits = guild_hits + 1
            WHERE guild_id = $1 AND user_id IS NULL;
        """, guild_id)


        row = await self.bot.db.fetchrow("""
            SELECT hits, (SELECT guild_hits FROM vape_hits WHERE guild_id = $1 AND user_id IS NULL) AS guild_hits
            FROM vape_hits
        ...
        await message.edit(content=None, embed=discord.Embed(
            description=f"> <:hits:1303239575250014241> {ctx.author.mention} took their {self.bot.ordinal(hits)} hit! "
                        f"the guild now has {guild_hits} hits"
        ))
    ...
    async def vape_hits(self, ctx):
        ...
        await ctx.send(f"{ctx.author.mention} has taken {self.bot.ordinal(hits)} hits.")
