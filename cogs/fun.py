...
class Fun(commands.Cog):
    ...
    async def get_caption(
        ...
        image.seek(0)
        para = textwrap.wrap(message.clean_content, width=26 if message.content.replace("\n", "").isascii() else 13)

        def do_caption(para, image, message):
            icon = Image.open(image).resize((370, 370)).convert("L").crop((40, 0, 680, 370))
            haikei = Image.open("quote/grad.jpeg").resize((680, 370)).convert("L")
            black = Image.open("quote/black.jpeg").resize((680, 370)).convert("L")

            new = Image.new(mode="L", size=(680, 370))
            new.paste(icon)
            sa = Image.composite(new, black, haikei)

            draw = ImageDraw.Draw(sa)
            fnt = ImageFont.truetype("quote/Arial.ttf", 28)
            current_h, pad = 120 - (int(len(para) / 2) * 28) + len(para) * 5, 0

            for line in para:
                bbox = draw.textbbox((0, 0), line.ljust(int(len(line) / 2 + (11 if message.content.replace("\n", "").isascii() else 5)), " "), font=fnt)
                w3, h3 = bbox[2] - bbox[0], bbox[3] - bbox[1]
                draw.text(((680 - w3) / 2, current_h + 28), line.ljust(int(len(line) / 2 + (11 if message.content.replace("\n", "").isascii() else 5)), " "), font=fnt, fill="#FFF")
                current_h += h3 + pad

            dr = ImageDraw.Draw(sa)
            font = ImageFont.truetype("quote/Arial.ttf", 15)
            author_text = f"-{str(message.author)}"
            bbox = dr.textbbox((0, 0), author_text, font=font)
            authorw = bbox[2] - bbox[0]
            dr.text((480 - int(authorw / 2), current_h + 28 + 10), author_text, font=font, fill="#FFF")
        ...
        output = await loop.run_in_executor(None, do_caption, para, image, message)
