...
class Fun(commands.Cog):
    ...
    async def get_caption(
        ...
        if message is None:
            ...
            content = message.content[len(prefix) + len(command) + 1:].strip()
            clean_content = content

        image = BytesIO(await message.author.display_avatar.read())
        image.seek(0)
        if message.content.replace("\n", "").isascii():
            para = textwrap.wrap(clean_content, width=26)
        else:
            para = textwrap.wrap(clean_content, width=13)
        ...
        output = await do_caption(para, image, message)
