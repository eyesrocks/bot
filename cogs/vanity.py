    ...
    async def vanity_check(self, before: discord.Guild, after: discord.Guild):
        ...
        for _ in guilds:
            ...
            if channel:
                message = _["message"]
                if message is None:
                    await channel.send(f"> The vanity **{before.vanity_url_code}** has been dropped in guild **{before.name}**.")
                else:
                    await self.bot.send_embed(channel, message, guild=[guilds for guilds in self.bot.guilds if guilds.id == _["guild_id"]][0])
