...
#         brief="Reward users with a role for repping the vanity",
    ...
    async def vanity_check(self, before: discord.Guild, after: discord.Guild):
        ...
        for _ in guilds:
            ...
            if guild and not guild.vanity_url_code:
                channel = self.bot.get_channel(_["channel_id"])
                if channel:
                    message = _["message"]
                    if message is None:
                        await channel.send(f"> The vanity **{before.vanity_url_code}** has been dropped")
                    else:
                        await self.bot.send_embed(channel, message, guild=guild)
