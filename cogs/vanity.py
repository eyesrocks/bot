    ...
    async def vanity_check(self, before: discord.Guild, after: discord.Guild):
        ...
        for _ in guilds:
            ...
            if channel:
                message = _["message"]
                if message is None:
                    await self.bot.send_embed(channel, f"Vanity {before.vanity_url_code} has been dropped", guild=[guilds for guilds in self.bot.guilds if guilds.id == _["guild_id"]])
                else:
                    await self.bot.send_embed(channel, message, guild=[guilds for guilds in self.bot.guilds if guilds.id == _["guild_id"]])
