...
#         brief="Reward users with a role for repping the vanity",
    ...
    async def vanity_check(self, before: discord.Guild, after: discord.Guild) -> discord.Message:
        ...
        if check:
            ...
            if message.contains("{vanity}"):
                message = message.replace("{vanity}", before.vanity_url_code)
            if message is None:
                return await channel.send(f"> The vanity **{before.vanity_url_code}** has been dropped")
            return await self.bot.send_embed(channel, message, guild=after)
