...
#         brief="Reward users with a role for repping the vanity",
    ...
    async def vanity_check(self, before: discord.Guild, after: discord.Guild):
        """
        Checks if a guild's vanity URL code has changed and notifies relevant channels.
        """
        ...
        for guild in guilds:
            ...
            message = guild.get("message") or f"Vanity {before.vanity_url_code} has been dropped"
            message = message.replace("{vanity}", before.vanity_url_code)
            
            try:
                await self.bot.send_embed(channel, message, user=after.owner)
            except Exception as e:
                logger.error(f"Failed to send vanity update message: {e}")
