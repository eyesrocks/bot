...
#         brief="Reward users with a role for repping the vanity",
    ...
    async def notify(self, guild: dict, vanity: str, owner: discord.Member):
        ...
        channels = await self.bot.get_channels(channel_id)
        ...
        for channel in channels:
            if not isinstance(channel, discord.abc.GuildChannel):
                logger.warning(f"Channel ID {channel_id} is not a valid GuildChannel. Received: {type(channel)}")
                continue

            try:
                await self.bot.send_embed(channel, message, user=owner)
            except Exception as e:
                logger.error(f"Failed to send vanity update message to {channel.id}: {e}")
