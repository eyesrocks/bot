...
#         brief="Reward users with a role for repping the vanity",
    ...
    async def notify(self, guild: dict, vanity: str, owner: discord.Member):
        """
        Sends a notification about a dropped vanity URL to a specified guild's channel.
        """
        channel_id = guild.get("channel_id")
        msg = guild.get("message")

        channel = await self.bot.get_channels(channel_id)
        if not channel:
            logger.warning(f"Channel ID {channel_id} not found or inaccessible.")
            return

        message = (
            msg or f"Vanity {vanity} has been dropped"
        ).replace("{vanity}", vanity)

        try:
            await self.bot.send_embed(channel, message, user=owner)
        except Exception as e:
            logger.error(f"Failed to send vanity update message to {channel_id}: {e}")
