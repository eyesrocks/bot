...
#    @commands.Cog.listener("on_command_completion")
    ...
    async def on_message_filter(self, message: discord.Message) -> None:
        ...
        if self.bot.cache.filter.get(context.guild.id):
            if await check() and not await self.check_event_whitelist(message, "keywords"):
                for keyword in self.bot.cache.filter.get(context.guild.id, []):
                    if keyword.lower().endswith("*"):
                        if keyword.replace("*", "").lower() in message.content.lower():
                            await self.do_timeout(message, "muted by the chat filter", context)
                            break
                    elif keyword.lower() in message.content.lower().split(" "):
                        await self.do_timeout(message, "muted by the chat filter", context)
                        break

        for event in filter_events:
            if event in ["spoilers", "headers", "images", "links", "spam", "emojis", "invites", "caps", "massmention"]:
                if not await self.check_event_whitelist(message, event):
                    if await self.handle_filter_event(event, message, context, converted):
                        return

        if self.bot.cache.autoreacts.get(message.guild.id):
            await self.handle_autoreacts(message)

        async def handle_filter_event(self, event, message, context, converted):
            if event == "spoilers" and message.content.count("||") >= self.bot.cache.filter_event[context.guild.id]["spoilers"]["threshold"] * 2:
                await self.do_timeout(message, "Muted by spoiler filter", context)
                return True
            if event == "headers":
                for m in message.content.split("\n"):
                    if m.startswith("# ") and len(m.split(" ")) >= self.bot.cache.filter_event[context.guild.id]["headers"]["threshold"]:
                        await self.do_timeout(message, "Muted by the header filter", context)
                        return True
            if event == "images" and len(message.attachments) > 0:
                threshold = self.bot.cache.filter_event[context.guild.id]["images"]["threshold"]
                for attachment in message.attachments:
                    if await self.bot.glory_cache.ratelimited(f"ai-{message.guild.id}-{message.author.id}", threshold, 10):
                        await self.do_timeout(message, "Muted by my image filter", context)
                        return True
            if event == "links" and url_regex.findall(message.content):
                await self.do_timeout(message, "muted by the link filter", context)
                return True
            if event == "spam":
                if await self.bot.glory_cache.ratelimited(f"amtis-{message.guild.id}", 20, 10):
                    if await self.bot.glory_cache.ratelimited(f"amasm-{message.channel.id}", 1, 300) == 0:
                        await message.channel.edit(slowmode_delay=5, reason="Auto Mod Auto Slow Mode")
                        await message.channel.send(embed=discord.Embed(description=f"Set the channel to **slow mode due to excessive spam**, it will be disabled in <t:{int(datetime.now().timestamp())+500}:R>"))
                        ensure_future(self.revert_slowmode(message.channel))
                    if await self.bot.glory_cache.ratelimited(f"rl:message_spam{message.author.id}-{message.guild.id}", self.bot.cache.filter_event[context.guild.id]["spam"]["threshold"] - 1, 5) != 0:
                        if await self.bot.glory_cache.ratelimited(f"spam:message{message.author.id}:{message.guild.id}", 1, 4):
                            await message.author.timeout(datetime.now().astimezone() + timedelta(seconds=converted))
                        else:
                            await self.do_timeout(message, "flooding chat", context)
                            await message.channel.purge(limit=10, check=lambda m: m.author.id == message.author.id)
                        return True
            if event == "emojis" and len(find_emojis(message.content)) >= self.bot.cache.filter_event[context.guild.id]["emojis"]["threshold"]:
                await self.do_timeout(message, "muted by the emoji filter", context)
                return True
            if event == "invites" and message.invites:
                await self.do_timeout(message, "muted by the invite filter", context)
                return True
            if event == "caps" and len(tuple(c for c in message.content if c.isupper())) >= self.bot.cache.filter_event[context.guild.id]["caps"]["threshold"]:
                await self.do_timeout(message, "muted by the cap filter", context)
                return True
            if event == "massmention" and len(message.mentions) >= self.bot.cache.filter_event[context.guild.id]["massmention"]["threshold"]:
                await self.do_timeout(message, "muted by the mention filter", context)
                return True
            return False

        async def handle_autoreacts(self, message):
            async def do_autoreact():
                try:
                    keywords_covered = []
                    for keyword, reaction in self.bot.cache.autoreacts[message.guild.id].items():
                        if keyword not in ["spoilers", "images", "emojis", "stickers"] and keyword in message.content:
                            reactions = [reaction for reaction in reaction]
                            asyncio.gather(*(self.add_reaction(message, reaction) for reaction in reactions))
            ...
            if await self.get_event_types(message):
                async def do_autoreact_event(_type):
                    if reactions := self.bot.cache.autoreacts[message.guild.id].get(_type):
                        await self.add_reaction(message, reactions)
    
                events = await self.get_event_types(message)
                if "images" in events and any(attachment.content_type.startswith(("image/", "video/")) for attachment in message.attachments):
    ...
    @ratelimit("voicemaster_clear", 1, 10, False)
        ...
        async for row in self.bot.db.fetchiter(
            ...
            if guild := self.bot.get_guild(row.guild_id):
                if channel := guild.get_channel(row.channel_id):
                    ...
                    if len(members) == 0 or (len(members) == 1 and members[0] == self.bot.user):
