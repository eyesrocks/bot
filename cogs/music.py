...
from typing import Optional, Union, Literal, Coroutine  # type: ignore
...
async def get_player(
    ...
    *,
    ...
    if (
        ...
        and interaction.guild.me.voice.channel != interaction.user.voice.channel
    ):
        raise commands.CommandError(
    ...
    ) is None or not interaction.guild.me.voice:
        if not connect:
            if interaction.voice_client:
                return await interaction.voice_client.disconnect()

            raise commands.CommandError("I'm not **connected** to a voice channel")
        else:
            await interaction.user.voice.channel.connect(cls=Player, self_deaf=True)
            player = interaction.client.node.get_player(interaction.guild.id)
            try:
...
async def enqueue(bot: Greed, interaction: discord.Interaction, query: str):
    try:
        ...
        raise e
    if player:
        if player.channel.id == interaction.voice_client.channel.id:
            try:
                result = await interaction.voice_client.node.get_tracks(
                    query=query, ctx=None, search_type=pomice.SearchType.scsearch
                )
                ...
                if match := regex.SOUNDCLOUD_TRACK_URL.match(
                    ...
                    try:
                        result = await player.node.get_tracks(
                            query=f"ytsearch:{match.group('slug')}", ctx=None
                        )
                    except Exception:
            ...
            if not result:
                ...
                track = result[0]
                await player.insert(track, bump=False)
...
async def play(bot: Greed, interaction: discord.Interaction):
    if player := bot.node.get_player(interaction.guild.id):
        ...
        try:
            if player.current.requester == interaction.user:
                embed = discord.Embed(
...
class Player(pomice.Player):
    ...
    def volume_bar(self) -> str:
        ...
        for i in range(filled_slots):
            if i > 9:
                ...
                bar[i] = filled
        if int(str(percentage)[-1]) >= 4:
            bar[filled_slots] = faded
        return "".join(bar)
    ...
    async def play(self, track: pomice.Track):
        ...
        try:
            ...
            if self.context:
                ...
                _ = await self.context.fail(
                    f"Could not play **[{track.title}]({track.uri})**"
                )
        if len(self.queue._queue) > 0:
            try:
                self.queue._queue.remove(track)
                try:
                    await self.play(self.queue._queue[1])
                except Exception:
                    pass
            except Exception:
                try:
            ...
            logger.info(
                f"playing {track} errored and the queue {self.queue._queue} was to short"
            )
    ...
    async def next_track(self, ignore_playing: bool = False):
        ...
        _ = await self.play(track)
        if isinstance(_, discord.Message):
            if len(self.queue._queue) > 0:
                try:
                    self.queue._queue.remove(track)
                    try:
                        self.track = self.queue._queue[0]
                        track = self.track
        ...
        if self.bound_channel and self.loop != "track":
            try:
                ...
                self.message = await track.ctx.send(embed=embed)
            except Exception:
...
class Music(commands.Cog):
        ...
        if not hasattr(self.bot, "node"):
            ...
            if spotify is None:
                ...
                self.bot.node = await pomice.NodePool().create_node(
                    ...
                    identifier=f"MAIN{tuuid()}",
                    spotify_client_id=spotify.get("c5ea934dfe2a47a68047629dba9e6aaf"),
                    spotify_client_secret=spotify.get(
                        "536dac49c02c4de4bc210a2c29ebb547"
                    ),
                    apple_music=True,
                )
                # except Exception as e:
    ...
    async def get_tracks(
        ...
        try:
            if results := await player.get_tracks(
                query=query, ctx=ctx
            ):
                return results
        except Exception:
            pass
        return await player.get_tracks(
            query=query, ctx=ctx, search_type=pomice.SearchType.scsearch
        )
    ...
    async def playing(
        ...
        if not player:
            return await ctx.fail("no current music session")
        if player.current:
            ...
            # embed.add_field(name = "Volume", value = player.volume_bar, inline = False)
            if player.current.thumbnail:
                embed.set_thumbnail(url=player.current.thumbnail)

            return await ctx.send(embed=embed, view=MusicInterface(self.bot))
        else:
            return await ctx.fail("no current track playing")
    ...
    async def play(self, ctx: Context, *, query: Optional[str] = None):
        if not ctx.author.voice: return await ctx.fail("you aren't in a voice channel")
        try:
            _ = await self.__play(ctx, query)
        except Exception as e:
            _ = None
            if ctx.author.name == "aiohttp" or "aiomisc":
                raise e
        try:
            if not _ and not ctx.voice_client.is_playing:
                return await ctx.fail("No track could be found")
        except Exception:
            return
            return await ctx.fail("No track could be found")

    async def __play(self, ctx: Context, query: Optional[str] = None):
        """Queue a track"""
        ...
        if query is None:
            if ctx.invoked_with in ("play", "p"):
                ...
                elif isinstance(query, discord.Message):
                    return
                search_type = None
        ...
        if query.lower() in ["-liked", "--liked", "liked"]:
            ...
            for track, artist in await self.bot.db.fetch(
                ...
                try:
                    result = await self.get_tracks(ctx, player, query)
                    if len(result) > 0:
                        await player.insert(
            ...
            if len(results) > 0:
                return await self.bot.paginate(
                    ctx,
                    discord.Embed(
                        title="Tracks Queued",
                        url=self.bot.domain,
            ...
            try:
                result = await self.get_tracks(ctx, player, query, search_type)
            except pomice.TrackLoadError:
                if match := regex.SOUNDCLOUD_TRACK_URL.match(
                    query
                ) or regex.SOUNDCLOUD_PLAYLIST_URL.match(query):
            ...
            if not result:
                ...
                if player.is_playing:
                    ...
                    if player.is_paused:
                        return await ctx.success(
                            f"Added [**{track.title}**]({track.uri}) to the queue but the player is currently **PAUSED**",
                            emoji="<a:greed_music:1207661059989831681>",
                        )
    ...
    @commands.command(
        name="remove",
        brief="remove a song from the order it was placed in",
        example=",remove 3",
        aliases=["rmv"],
    )
    ...
    @commands.command(
        ...
        aliases=["vol", "v"],
        example=",volume 50",
        brief="Adjust the volume of greed playing music",
    )
    async def volume(self, ctx: Context, percentage: int = 65):
        ...
        if percentage is None:
            return await ctx.success(f"Current volume: `{player.volume}%`")
    ...
    @commands.command(
        ...
        aliases=["dc", "stop"],
        example=",disconnect",
        brief="Make greed leave your current voice channel",
    )
