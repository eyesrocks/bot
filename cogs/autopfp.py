...
class AutoPFP(Cog):
    ...
    async def autopfp_add(self, ctx: Context, *, channel: TextChannel):
        ...
        if categories := state.get("categories"):
            await self.bot.db.execute("""INSERT INTO autopfp (guild_id, channel_id, categories) VALUES($1, $2, $3) ON CONFLICT(guild_id, channel_id, categories) DO UPDATE SET categories = excluded.categories""", ctx.guild.id, channel.id, categories)
