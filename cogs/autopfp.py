...
class AssetResponse(BaseModel):
    ...
    posted: Optional[List[int]] = []

    @classmethod
    async def from_category(cls: Type["AssetResponse"], endpoint: str, category: str) -> "AssetResponse":
        while True:
            async with Session() as session:
                async with session.get(f"https://cdn.rival.rocks/{endpoint}", params = {"category": category, "tuuid": tuuid()}) as response:
...
class AutoPFP(Cog):
    ...
    async def send_avatars(self, channel: TextChannel, categories: List[str], avatars: Dict[str, AssetResponse]):
        message = None
        embeds = [avatars[c].to_embed(channel.guild) for c in categories if channel.id not in avatars[c].posted]
        new_embeds = []
        urls = []
        for embed in embeds:
        ...
        for category in categories:
            avatars[category].posted.append(channel.id)
