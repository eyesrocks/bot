...
class Socials(commands.Cog):
    ...
    def extract_data(self, url: URL | str, **params) -> Optional[Munch]:
        try:
            data = self.ytdl.extract_info(url=str(url), download=False, **params)
            return DefaultMunch.fromDict(data) if data else None
    ...
    async def on_youtube_request(self, ctx: Context, url: str):
        data = await self.extract_data(url, force_generic_extractor=True)
        if not data:
            return
        ...
        try:
            ...
            logger.error(f"Youtube request: {e}")
            return
