...
class Moderation(Cog):
    ...
    async def role_all_task(
        ...
        # Filter valid members based on parameters
            if mentionable:
            ...
            members = [
                ...
                if (
                    m != ctx.guild.me and
                    m.top_role < ctx.author.top_role and 
                    m.top_role < ctx.guild.me.top_role and
                    role.is_assignable() and
                    (m.bot if bots else not m.bot) and
                    (role in m.roles if remove else role not in m.roles)
                )
            ]
            ...
            for chunk in member_chunks:
                ...
                for member in chunk:
                    if remove:
                        tasks.append(member.remove_roles(role, reason=f"Mass role removal by {ctx.author}"))
                    else:
                        tasks.append(member.add_roles(role, reason=f"Mass role addition by {ctx.author}"))
                        
                await asyncio.gather(*tasks, return_exceptions=True)
                processed += len(chunk)
                
                # Update status message every 50 members
                if processed % 50 == 0:
                    await message.edit(
                        embed=discord.Embed(
                            color=self.bot.color,
