...
class Moderation(Cog):
    ...
    async def warn(self, ctx: Context, member: Member, *, reason: str) -> discord.Message:
        if not self.bot.hierarchy(ctx, member, author=True):
            return
        await self.bot.db.execute(
    ...
    async def warn_list(self, ctx: Context, member: discord.Member) -> discord.Message:
        warnings = await self.bot.db.fetch(
            """SELECT reason, moderator_id FROM warnings WHERE guild_id = $1 AND user_id = $2""",
            ctx.guild.id,
            member.id,
        )
        if not warnings:
        ...
        for i, warning in enumerate(warnings, start=1):
            ...
            rows.append(f"`{i}` **{warning['reason']}** by **{moderator}**")
        return await self.bot.dummy_paginator(
            ctx,
            discord.Embed(title=f"Warnings for {member}", color=self.bot.color),
            rows,
            type="warnings",
        )
    ...
    @commands.has_permissions(manage_messages=True)
    async def warn_remove(self, ctx: Context, member: discord.Member, index: int) -> discord.Message:
        warnings = await self.bot.db.fetch(
            """SELECT reason, moderator_id FROM warnings WHERE guild_id = $1 AND user_id = $2""",
            ctx.guild.id,
            member.id,
        ...
        try:
            ...
            return await ctx.fail(f"Warning **{index}** doesn't exist")
        await self.bot.db.execute(
            """DELETE FROM warnings WHERE guild_id = $1 AND user_id = $2 AND reason = $3""",
            ctx.guild.id,
            member.id,
            warning["reason"],
        )
        return await ctx.success(f"Removed warning **{index}** from {member.mention}")
