...
**Banner:** {f"[Here]({guild.banner.url})" if guild.banner else 'None'}""",
    ...
    async def whois(self, ctx, user: Member = None):
        ...
        if devices != "none":
            primary_device = devices.split(", ")[0].strip()
            status_emoji = emojis.get(f"{primary_device}_{user.status.name.lower()}", "")
        ...
        if user.premium_since:
            embed.add_field(
                name="**Boosted server**",
                value=f"""> **{discord.utils.format_dt(round(user.joined_at.timestamp()), style='D')}**""",
            )
