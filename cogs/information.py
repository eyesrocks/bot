...
**Banner:** {f"[Here]({guild.banner.url})" if guild.banner else 'None'}""",
    ...
    async def whois(self, ctx, user: Member = None):
        ...
        emojis = {
            ...
            "mobile_dnd": "<:status_dnd:1302238595515551784>",
            "mobile_idle": "<:status_idle:1302238592172822569>",
            "mobile_online": "<:status_online:1302238596580773980>",
            "web_offline": "",
            "mobile_offline": "",
            "desktop_offline": "",
        ...
        for staff_flagg in (
            ...
            if getattr(staff_flags, staff_flagg, False) is True:
                staff.append(_staff[staff_flagg])
        
        def founder(u):
            if u.id in self.bot.lim:
                return True
            return False

        def is_bot_staff(u):
            if u.id in self.bot.owner_ids:
                return True
            return False

        def is_dev(u):
            if u.id in self.bot.dev:
                return True
            return False
...
**Managed:** {role.managed}""",
    ...
    async def purchase(self, ctx):
        ...
        monthly_button = Button(label="Monthly - $3.50", style=discord.ButtonStyle.green, custom_id="https://buy.stripe.com/aEUdUJc8w3uf6qsfZ7")
        ...
        async def button_callback(interaction):
            if interaction.custom_id == "monthly":
                ...
                await interaction.response.send_message("You selected the Lifetime plan ($6). Please open a ticket to proceed.", ephemeral=True)
