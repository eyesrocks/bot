...
class AvatarHistory(Cog):
    ...
    async def avatarhistory(self, ctx: Context, *, user: Optional[Union[User, Member]] = None):
        ...
        for i, row in enumerate(rows, start=1):
            url = f"https://cdn.greed.wtf/avatars/{user.id}/{row['id']}"
            embed.add_field(name=f"Avatar {i}/{len(rows)}", value=f"[Link]({url}) - changed {utils.format_dt(row['ts'], style='R')}", inline=False)
