...
class Antiraid(commands.Cog):
    ...
    async def log_raid_activity(self, reason, member, guild):
        ...
        if log_channel_id:
            ...
            if log_channel:
                await log_channel.send(f"**[Raid Log]** {member} attempted to join with reason: {reason}")
            else:
                await guild.system_channel.send(f"Log channel with ID {log_channel_id} no longer exists.")
        else:
    ...
    async def toggle_lockdown(self, ctx, status: str):
        ...
        for channel in ctx.guild.text_channels:
            ...
            if new_lockdown:
                ...
                if channel.id in self.original_permissions:
                    ...
                    await channel.set_permissions(ctx.guild.default_role, send_messages=original['send_messages'], read_messages=original['read_messages'])
        ...
        await ctx.success(f"Server is now {state}. New member joins are {'blocked' if new_lockdown else 'allowed'}.")
    ...
    async def status(self, ctx):
        ...
        embed.add_field(name="Antiraid System", value="<:UB_Check_Icon:1306875712782864445>" if settings["antiraid_enabled"] else "<:UB_X_Icon:1306875714426900531>", inline=False)
        embed.add_field(name="Lockdown", value="<:UB_Check_Icon:1306875712782864445>" if settings["lockdown"] else "<:UB_X_Icon:1306875714426900531>", inline=False)
    ...
    async def on_member_join(self, member):
        ...
        if account_age < settings["minimum_account_age"]:
            await self.handle_account_age(member, account_age, settings['minimum_account_age'])
