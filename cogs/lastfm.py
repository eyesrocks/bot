...
class LastFM(commands.Cog):
    ...
    @lastfm.command(
        name="spotify",
        brief="Get a Spotify link to the current track",
        example=",lastfm spotify",
    )
    async def lastfm_spotify(self, ctx: Context):
        if not (
            conf := await self.bot.db.fetchrow(
                "SELECT * FROM lastfm.conf WHERE user_id = $1", ctx.author.id
            )
        ...
        if "error" in eggstra:
            return await ctx.fail("Invalid track.")
        if not eggstra["track"]["toptags"]["tag"]:
            return await ctx.fail("No Spotify link found.")
        for tag in eggstra["track"]["toptags"]["tag"]:
            if tag["name"] == "spotify":
                return await ctx.success(f"**[Spotify Link]({tag['url']})**")
    ...
    async def fm(self, ctx: Context):
        return await ctx.invoke(self.lastfm_nowplaying)
