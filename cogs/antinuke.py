from discord.ext.commands import Cog, command, Context, check, hybrid_group, bot_has_permissions
from discord import (
    AuditLogAction,
    AuditLogEntry,
    Member,
...
class AntiNuke(Cog):
    ...
    async def hardban(self, ctx: Context, user: User):
        ...
        if res:
            await ctx.confirm("user is already hardbanned, do you want to unhardban?")
            if await self.bot.db.fetchval(
                """SELECT user_id FROM hardban WHERE user_id = $1""", user.id
            ):
                await self.bot.db.execute(
                    """DELETE FROM hardban WHERE user_id = $1""", user.id
                )
                return await ctx.success(f"successfully **unhardbanned** {user.mention}")
        await self.bot.db.execute(
            """INSERT INTO hardban (user_id) VALUES($1)""", user.id
        )
