...
class AntiNuke(Cog):
    ...
    async def get_thresholds(self, guild: Guild, action: Union[AuditLogAction, str]) -> Optional[int]:
        if guild.id in self.guilds:
            ...
            if threshold is not None:
                return int(threshold) if int(threshold) != 1 else 0
            if action_str in self.thresholds[guild.id]:
                thres = self.thresholds[guild.id].get(action_str)
                return thres if thres != 1 else 0
        return 0

    async def do_ban(self, guild: Guild, user: Union[User, Member], reason: str):
        with suppress(TypeError):
            if hasattr(user, "top_role") and (user.top_role >= guild.me.top_role or user.id == guild.owner_id):
                raise TypeError("User's role is higher than mine or user is the owner")
    ...
    async def do_kick(self, guild: Guild, user: Union[User, Member], reason: str):
        async with self.locks[guild.id]:
            if hasattr(user, "top_role") and (user.top_role.position >= guild.me.top_role.position or user.id == guild.owner_id):
                return False
            if await self.bot.glory_cache.ratelimited(f"punishment-{guild.id}-{user.id}", 1, 60) != 0:
                return
    ...
    async def do_punishment(self, guild: Guild, user: Union[User, Member], reason: str):
        ...
        if user.bot or punishment.lower() == "ban":
    ...
    async def check_entry(self, guild: Guild, entry: AuditLogEntry):
        if entry.user is not None:
            threshold = await self.get_thresholds(guild, entry.action) or 0
            if await self.bot.db.fetchval(
                ...
                return True
            if entry.user.id in {guild.owner_id, self.bot.user.id} or entry.user.id in self.bot.owner_ids or (hasattr(entry.user, "top_role") and entry.user.top_role >= guild.me.top_role):
                return True
            ...
            if rl == 0 and "bot" not in str(entry.action).lower() and "guild_update" not in str(entry.action).lower():
                ...
                logger.info(f"user {entry.user.name} passed the threshold of {threshold} with {entry.action}")
        return False

    def check_guild(self, guild: Guild, action: Union[AuditLogAction, str]):
        action_str = get_action(action) if isinstance(action, AuditLogAction) else action
        return action_str in self.guilds.get(guild.id, {})
    ...
    async def get_audit(self, guild: Guild, action: AuditLogAction = None):
        ...
        try:
            audit = [
                a async for a in guild.audit_logs(
                    limit=1,
            ...
            if audit.user_id == self.bot.user.id and "|" in audit.reason:
                audit.user = self.bot.get_user(int(audit.reason.split(" | ")[-1].strip()))
            return audit
        except Exception:
            return None
    ...
    async def on_member_action(self, entry: AuditLogEntry):
        if not self.check_guild(entry.guild, entry.action):
            return
        reason = self.make_reason(f"User caught {entry.action.name.lower().replace('_', ' ')} members")
    ...
    async def dangerous_role_assignment(self, before: Member, after: Member):
        if not before.guild.me.guild_permissions.view_audit_log or before.roles == after.roles:
            return
        if not self.check_guild(after.guild, "role_update"):
            return
        ...
        if entry and not await self.check_entry(after.guild, entry):
            for r in new_roles:
                await after.remove_roles(r, reason=self.make_reason("User given roles with permissions"))
            await self.do_punishment(after.guild, entry.user, self.make_reason("User caught giving roles with permissions"))

    @Cog.listener("on_guild_role_update")
    async def role_update(self, before: Role, after: Role):
        if not before.guild.me.guild_permissions.view_audit_log or not await self.check_role(after):
            return
        if not self.check_guild(after.guild, "role_update"):
            return
        entry = await self.get_audit(after.guild, AuditLogAction.role_update)
        if entry and not await self.check_entry(after.guild, entry):
            await after.edit(permissions=Permissions(before.permissions.value), reason=self.make_reason("Cleanup"))
            await self.do_punishment(after.guild, entry.user, self.make_reason("User caught giving roles dangerous permissions"))

    @Cog.listener("on_guild_role_delete")
    async def role_delete(self, role: Role):
        if not self.check_guild(role.guild, "role_update"):
            return
        entry = await self.get_audit(role.guild, AuditLogAction.role_delete)
        if entry and not await self.check_entry(role.guild, entry):
            await role.clone(reason=self.make_reason("Cleanup"))
            await self.do_punishment(role.guild, entry.user, self.make_reason("User caught deleting roles"))
    ...
    async def role_create(self, role: Role):
        ...
        if entry and not await self.check_entry(role.guild, entry):
            ...
            await self.do_punishment(role.guild, entry.user, self.make_reason("User caught creating roles"))

    @Cog.listener("on_guild_channel_create")
    async def channel_create(self, channel):
        if not self.check_guild(channel.guild, "channel_update"):
            return
        entry = await self.get_audit(channel.guild, AuditLogAction.channel_create)
        if entry and not await self.check_entry(channel.guild, entry):
            await channel.delete(reason=self.make_reason("Cleanup"))
            await self.do_punishment(channel.guild, entry.user, self.make_reason("User caught creating channels"))

    @Cog.listener("on_guild_channel_delete")
    async def channel_delete(self, channel):
        if not self.check_guild(channel.guild, "channel_update"):
            return
        entry = await self.get_audit(channel.guild, AuditLogAction.channel_delete)
        if entry and not await self.check_entry(channel.guild, entry):
            await channel.clone(reason=self.make_reason("Cleanup"))
            await self.do_punishment(channel.guild, entry.user, self.make_reason("User caught deleting channels"))

    @Cog.listener("on_guild_channel_update")
    async def channel_update(self, before, after):
        if not self.check_guild(after.guild, "channel_update"):
            return
        entry = await self.get_audit(after.guild, AuditLogAction.channel_update)
        if entry and not await self.check_entry(after.guild, entry):
            await after.edit(
                name=before.name,
            ...
            await self.do_punishment(after.guild, entry.user, self.make_reason("User caught updating channels"))

    @Cog.listener("on_webhook_update")
    async def webhooks(self, channel):
        if not self.check_guild(channel.guild, "webhooks"):
            return
        entry = await self.get_audit(channel.guild, AuditLogAction.webhook_create)
        if entry and not await self.check_entry(channel.guild, entry):
            with suppress(Exception):
                await entry.target.delete(reason=self.make_reason("Cleanup"))
            await self.do_punishment(channel.guild, entry.user, self.make_reason("User caught creating webhooks"))

    @Cog.listener("on_member_join")
    async def antibot(self, member: Member):
        if not member.bot or not self.check_guild(member.guild, "bot_add"):
            return
        entry = await self.get_audit(member.guild, AuditLogAction.bot_add)
        if entry and not await self.check_entry(member.guild, entry):
            await member.ban(reason=self.make_reason("Cleanup"))
            await self.do_punishment(member.guild, entry.user, self.make_reason("User caught adding bots"))
