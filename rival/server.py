...
class Server:
    ...
    If the library is installed using PyPi, you can also use terminal to start server using `rival --port 1234`
    ...
    def __on_client_disconnect(self, client, _):
        self.disconnect_client(client, _)
        self.write_json()


    def disconnect_client(self, client, _):
    ...
    def write_json(self):
        ...
        with open("/root/clients.json", "wb") as file:
            file.write(orjson.dumps(clients))
        logger.info(f"wrote the clients to /root/clients.json")

    def __on_message(self, client, _, msg):
        try:
            msg = WsMessage(msgspec.json.decode(msg))
        except Exception:
            msg = WsMessage(msgspec.json.decode(msg.decode("UTF-8")))
        ...
        if msg.type.client_list:
            logger.info("Received Client List Message from client %s" % client['address'][1])
