...
class Handler:
    ...
    async def get_embed(
        ...
        if isinstance(ctx, Context):
            ...
            # elif event == EventType.untime_out:
            ...
            #     embed.description = f"**Moderator:** {ctx.author.mention}\n> **User:** {str(args.get('user','member'))}\n> **Punishment:** member untimed out\n> **When:** {ts}"
                ...
                if r := args.get("args"):
                    ...
                    role = args.get("role")
                if role is None:
                    logger.warning("Role is None; cannot access 'name'.")
                    embed.description = "**Role information is unavailable**"
                else:
    ...
    async def handle_log(
        ...
        if isinstance(ctx, Message):
            ...
            if ctx.command.qualified_name == "role":
                ...
                val = EventType.kick
            elif ctx.command.qualified_name == "reactionmute":
                role = discord.utils.get(ctx.guild.roles, name="rmute")
                member = ctx.kwargs.get("member")
                if role in member.roles:
                    val = EventType.reaction_mute
                else:
                    val = EventType.reaction_unmute
            elif ctx.command.qualified_name == "imagemute":
                role = discord.utils.get(ctx.guild.roles, name="imute")
                if role in ctx.kwargs.get("member").roles:
                    val = EventType.image_mute
                else:
                    val = EventType.image_unmute   
            ...
            if action == 10:
                channel_type = str(ctx.target.type)
                ...
                channel_type = str(ctx.target.type)
                ct = str(ctx.changes.before.type)
                if channel_type == "category" or ct == "category":
                    val = EventType.category_channel_delete
                else:
                    val = EventType.channel_delete
