...
class IncidentHistory(BaseModel):
    ...
    async def from_feed(cls):
        ...
        for incident in data:
            ...
            incident["summary"] = re.sub(r'<.*?>', '', incident.pop("summary", "")).replace("PDT", "PDT ").replace(".", ". ")
            incident["summary_detail"]["value"] = re.sub(r'<.*?>', '', incident["summary_detail"].pop("value", "")).replace("PDT", "PDT ").replace(".", ". ")
            new.append(incident)
